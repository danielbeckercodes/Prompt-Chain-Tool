...
import type { HumorFlavorStepWithRelations } from "@/lib/types";
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="flex items-center gap-0.5 sm:gap-1 flex-shrink-0">
                ...
                {/* Delete */}
                <Button variant="icon" onClick={onDelete} title="Delete step" className="text-red-500 hover:text-red-700">
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
            ...
            {(step.llm_system_prompt || step.llm_user_prompt) && (
              <div className="mt-3 space-y-2">
                {step.llm_system_prompt && (
                  <div>
                    <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-0.5">System Prompt</p>
                    <p className="text-xs text-zinc-600 dark:text-zinc-400 font-mono bg-zinc-50 dark:bg-zinc-800 rounded-lg px-2.5 py-1.5 line-clamp-2">
                      {step.llm_system_prompt}
                    </p>
                  </div>
                )}
                {step.llm_user_prompt && (
                  <div>
                    <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-0.5">User Prompt</p>
                    <p className="text-xs text-zinc-600 dark:text-zinc-400 font-mono bg-zinc-50 dark:bg-zinc-800 rounded-lg px-2.5 py-1.5 line-clamp-2">
                      {step.llm_user_prompt}
                    </p>
                  </div>
